...
class DATESampling(Strategy):
    ...
    def train_xgb_model(self):
        ...
        self.xgb = XGBClassifier(n_estimators=100, max_depth=4, n_jobs=-1, eval_metric="error", scale_pos_weight = 1)
        self.xgb.fit(self.data.dftrainx_lab, self.data.train_cls_label)
        best_thresh, best_auc = find_best_threshold(self.xgb,self.data.dfvalidx_lab, self.data.valid_cls_label)
        xgb_test_pred = self.xgb.predict_proba(self.data.dftestx)[:,-1]
        overall_f1,auc,pr, re, f, rev = metrics(xgb_test_pred, self.data.test_cls_label,self.data.test_reg_label,best_thresh)
...
#         if torch.cuda.device_count() > 1:
    ...
    def evaluate(self):
        ...
        pred_path = str(type(self.data))[-7:-2] +"_" +str(self.data.args.initial_inspection_rate) + "%_prediction.csv"
        pred_ser = pd.Series(y_prob)
        pred_ser.to_csv(pred_path,index=0)
